...
// Generated: 2026-06-15T19:39:12.493Z
(function() {
  if (!VanillaReactive) {
...
(function() {
  ...
  const { pubSub } = VanillaReactive.services;
  class BindingReferenceComponent extends BaseComponent {
    constructor(ctx) {
      super(ctx);
    }
    get isBusy() {
    ...
    refreshTimestamp() {
      this.state.updatedAt = (/* @__PURE__ */ new Date()).toLocaleTimeString();
    }
    showNotification() {
      pubSub.publish("app-show-notification", { type: "info", message: "¡Botón clickeado!" });
    }
    render(changedProp) {
      if (changedProp && this.element) {
        const phaseEl = this.element.querySelector("[data-render-phase]");
        const propEl = this.element.querySelector("[data-last-prop]");
  ...
  registerComponent("app-binding-reference", BindingReferenceComponent);
})();
(function() {
  if (!VanillaReactive) {
    console.error("VanillaReactive no está definido. Asegúrate de incluir vanilla-reactive.iife.js antes de este script.");
  ...
  class ClockComponent extends BaseComponent {
    ...
    init() {
      ...
      const intervalId = setInterval(() => {
        update("date", (/* @__PURE__ */ new Date()).toTimeString().split(" ")[0]);
      }, 1e3);
...
(function() {
  ...
  class ComboBoxComponent extends BaseComponent {
    ...
    selectOption(_el, _ev, index) {
      ...
      this.selected?.(this.element, null, { id: item.id, label: item.label });
    }
    handleKeyDown(_el, ev) {
      const key = ev.key;
    ...
    renderList(el) {
      ...
      const listTemplate = `
        <div class="max-h-30 overflow-auto ${this.props.listClass ?? ""}">
      ...
      this.liItems = $("[data-combo-idx]", div).all();
      el.appendChild(div);
      this.applyHighlight();
    }
    initFloatingList() {
  ...
  registerComponent("combo-box-component", ComboBoxComponent);
})();
(function() {
  if (!VanillaReactive) {
    console.error("VanillaReactive no está definido. Asegúrate de incluir vanilla-reactive.iife.js antes de este script.");
...
(function() {
  ...
  class DashboardComponent extends BaseComponent {
    ...
    addPercent(el, ctx) {
      const sender = ctx.target;
      if (sender) el.style.width = sender.value + "%";
    }
    miMetodoPersonalizado(el, payload, extra) {
        ...
        <div class="my-8 p-8 bg-slate-50 rounded-3xl border border-slate-200 shadow-xl">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6"
            ...
            <div class="text-slate-400">
              Theme: <span on-publish="THEME_CHANGED:global:html"></span>
            </div>
            <button
          ...
          <footer class="mt-8 text-slate-400 font-bold uppercase tracking-widest">
  ...
  registerComponent("app-dashboard", DashboardComponent);
})();
(function() {
  class FloatingPortal {
    constructor(trigger, content, options = {}) {
      ...
      this.rafId = 0;
      this.isQueued = false;
      this.associatedElements = /* @__PURE__ */ new Set();
